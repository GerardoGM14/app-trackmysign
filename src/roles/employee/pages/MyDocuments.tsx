...
type RelationFilter = 'all' | 'creator' | 'signer';
...
interface DocumentItem {
    ...
    expiresAt?: string;
    fileSizeKB: number;
    /** Set on the doc when current user signed it (mock storage) */
    mySignature?: string;
    mySignedAt?: string;
}
...
const RELATION_TABS: { value: RelationFilter; label: string }[] = [
    { value: 'all', label: 'Todos' },
    { value: 'creator', label: 'Como creador' },
    { value: 'signer', label: 'Como firmante' },
];

// Current employee email (mock — would come from auth in real life)
const ME_EMAIL = 'employee@shop.com';
...
const INITIAL_DOCUMENTS: DocumentItem[] = [
    {
        ...
        signers: [
            { name: 'Anna Schmidt', email: ME_EMAIL, status: 'pending' },
            { name: 'Hans Weber', email: 'hans@miempresa.com', status: 'signed', signedAt: '2026-05-06' },
        ],
        createdBy: 'Hans Weber', createdAt: '2026-05-04', sentAt: '2026-05-04', expiresAt: '2026-05-15', fileSizeKB: 184,
    },
    {
        ...
        createdBy: 'Hans Weber', createdAt: '2026-05-01', sentAt: '2026-05-01', expiresAt: '2026-05-12', fileSizeKB: 421,
    },
    {
        id: 'D-2026-038', title: 'Anexo modificatorio v2', type: 'addendum', status: 'sent',
        signers: [{ name: 'Olaf Nilsen', email: 'olaf@partner.no', status: 'pending' }],
        createdBy: 'Anna Schmidt', createdAt: '2026-04-25', sentAt: '2026-04-25', expiresAt: '2026-05-09', fileSizeKB: 96,
    },
    {
        id: 'D-2026-035', title: 'Contrato laboral — Renovación', type: 'contract', status: 'signed',
        signers: [
            { name: 'Anna Schmidt', email: ME_EMAIL, status: 'signed', signedAt: '2026-04-26' },
            { name: 'Hans Weber', email: 'hans@miempresa.com', status: 'signed', signedAt: '2026-04-26' },
        ],
        createdBy: 'Hans Weber', createdAt: '2026-04-22', sentAt: '2026-04-22', completedAt: '2026-04-26', fileSizeKB: 312,
    },
    {
        id: 'D-2026-033', title: 'Acuerdo de colaboración Q2', type: 'agreement', status: 'signed',
        signers: [
            { name: 'María Silva', email: 'maria@partner.com', status: 'signed', signedAt: '2026-05-04' },
            { name: 'Anna Schmidt', email: ME_EMAIL, status: 'signed', signedAt: '2026-05-03' },
        ],
        createdBy: 'Anna Schmidt', createdAt: '2026-05-02', sentAt: '2026-05-02', completedAt: '2026-05-04', fileSizeKB: 248,
    },
    {
        id: 'D-2026-031', title: 'Política de seguridad interna', type: 'other', status: 'signed',
        signers: [{ name: 'Anna Schmidt', email: ME_EMAIL, status: 'signed', signedAt: '2026-04-15' }],
        createdBy: 'Hans Weber', createdAt: '2026-04-12', sentAt: '2026-04-12', completedAt: '2026-04-15', fileSizeKB: 78,
    },
    {
        id: 'D-2026-029', title: 'Borrador — NDA proveedor cloud', type: 'nda', status: 'draft',
        signers: [],
        createdBy: 'Anna Schmidt', createdAt: '2026-05-08', fileSizeKB: 145,
    },
    {
        id: 'D-2026-028', title: 'Anexo modificatorio expirado', type: 'addendum', status: 'expired',
        signers: [{ name: 'Klaus Bauer', email: 'klaus@partner.de', status: 'pending' }],
        createdBy: 'Anna Schmidt', createdAt: '2026-04-01', sentAt: '2026-04-01', expiresAt: '2026-04-15', fileSizeKB: 110,
    },
];
...
export default function MyDocuments() {
    ...
    const { user } = useAuth();
    const myEmail = user?.email ?? ME_EMAIL;
    const myName = ME_NAME;
    ...
    // Helpers — define what "I am the X" means
    const isCreatedByMe = (d: DocumentItem) => d.createdBy === myName;
    const isSignerInDoc = (d: DocumentItem) => d.signers.some((s) => s.email === myEmail);
    const mySignerEntry = (d: DocumentItem) => d.signers.find((s) => s.email === myEmail);

    const filteredDocs = useMemo(() => {
        const q = search.toLowerCase().trim();
        const filtered = docs.filter((d) => {
            // Always: only docs where I have some involvement
            const involved = isCreatedByMe(d) || isSignerInDoc(d);
            if (!involved) return false;

            // Relation tab
            if (relation === 'creator' && !isCreatedByMe(d)) return false;
            if (relation === 'signer' && !isSignerInDoc(d)) return false;
    ...
    }, [docs, search, relation, typeFilter, statusFilter, sortKey, sortDir]);

    useEffect(() => setCurrentPage(1), [search, relation, typeFilter, statusFilter]);
    ...
    // Stats — scoped to docs where I'm involved
    const stats = useMemo(() => {
        const myDocs = docs.filter((d) => isCreatedByMe(d) || isSignerInDoc(d));
        const pendingForMe = myDocs.filter(
            (d) => mySignerEntry(d)?.status === 'pending' && (d.status === 'pending' || d.status === 'sent'),
        ).length;
        return {
            total: myDocs.length,
            pendingForMe,
            signed: myDocs.filter((d) => d.status === 'signed').length,
            createdByMe: myDocs.filter(isCreatedByMe).length,
        };
        // eslint-disable-next-line react-hooks/exhaustive-deps
    }, [docs]);

    const filtersActive =
        search.trim() !== '' || typeFilter !== 'ALL' || statusFilter !== 'ALL' || relation !== 'all';
    ...
    const performDelete = (id: string) => {
        ...
        showToast(`Borrador "${target.id}" eliminado.`, 'success');
    };
    ...
    const handleConfirmSign = (signatureDataUrl: string) => {
        ...
        setDocs((prev) =>
            prev.map((d) => {
                if (d.id !== signingDoc.id) return d;

                const updatedSigners = d.signers.map((s) =>
                    s.email === myEmail
                        ? { ...s, status: 'signed' as const, signedAt: new Date().toISOString().split('T')[0] }
                        : s,
                );
                ...
                return {
                    ...
                    mySignature: signatureDataUrl,
                    mySignedAt: signedAt,
                    completedAt: allSigned ? new Date().toISOString().split('T')[0] : d.completedAt,
                };
            }),
        );
    ...
    const exportToExcel = async () => {
        ...
        try {
            ...
            const ws = workbook.addWorksheet('Mis documentos');
            ws.columns = [
                { header: 'ID', key: 'id', width: 14 },
                { header: 'Título', key: 'title', width: 40 },
                { header: 'Tipo', key: 'type', width: 14 },
                { header: 'Estado', key: 'status', width: 14 },
                { header: 'Mi rol', key: 'role', width: 16 },
                { header: 'Mi estado', key: 'mySignerStatus', width: 14 },
            ...
            filteredDocs.forEach((d) => {
                const me = mySignerEntry(d);
                const role = isCreatedByMe(d)
                    ? isSignerInDoc(d)
                        ? 'Creador y firmante'
                        : 'Creador'
                    : 'Firmante';
                ws.addRow({
                    ...
                    role,
                    mySignerStatus: me ? (me.status === 'signed' ? 'Firmado' : me.status === 'rejected' ? 'Rechazado' : 'Pendiente') : '—',
            ...
            saveAs(blob, `mis-documentos_${new Date().toISOString().split('T')[0]}.xlsx`);
        } catch (err) {
            console.error(err);
        ...
        <>
            <div className="space-y-5">
                ...
                <header className="flex flex-col lg:flex-row lg:items-end justify-between gap-4">
                    <div>
                        ...
                        <p className="text-sm text-slate-500 mt-1.5">
                            Documentos que has creado o donde participas como firmante.
                        </p>
                    </div>
                ...
                <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
                    ...
                    <StatCard label="Esperando mi firma" value={stats.pendingForMe} dot="bg-amber-500" />
                    <StatCard label="Firmados" value={stats.signed} dot="bg-emerald-500" />
                    <StatCard label="Que creé" value={stats.createdByMe} dot="bg-sky-500" />
                </div>
                    ...
                    <div className="inline-flex h-9 bg-slate-100 border border-slate-200 rounded-md p-0.5 self-start lg:self-auto">
                        {RELATION_TABS.map((tab) => (
                            <button
                                key={tab.value}
                                type="button"
                                onClick={() => setRelation(tab.value)}
                                className={`px-3 text-[12px] font-medium rounded transition-colors cursor-pointer flex items-center gap-1.5 ${relation === tab.value
                                        ? 'bg-white text-slate-900 border border-slate-200'
                                        : 'text-slate-500 hover:text-slate-900'
                                    }`}
                            >
                                {tab.value === 'creator' && <BsPersonBadge size={11} />}
                                {tab.value === 'signer' && <BsPersonCheck size={11} />}
                                {tab.label}
                            </button>
                        ))}
                    </div>
                        ...
                        <table className="w-full text-left text-sm">
                            <thead>
                                <tr className="bg-slate-50/60 border-b border-slate-200">
                                    ...
                                    <th className="px-4 py-2 text-[10.5px] font-semibold text-slate-500 uppercase tracking-wider">Mi rol</th>
                                    <th className="px-4 py-2 text-[10.5px] font-semibold text-slate-500 uppercase tracking-wider">Mi firma</th>
                            ...
                            <tbody>
                                {paginatedDocs.map((doc) => {
                                    ...
                                    const me = mySignerEntry(doc);
                                    const meCreator = isCreatedByMe(doc);
                                    const meSigner = isSignerInDoc(doc);
                                    const needsMySignature = me?.status === 'pending' && (doc.status === 'pending' || doc.status === 'sent');
                                    const alreadySigned = me?.status === 'signed';
                                    ...
                                    return (
                                        ...
                                        >
                                            ...
                                            <td className="px-4 py-2.5">
                                                <RoleBadge meCreator={meCreator} meSigner={meSigner} />
                                            </td>
                                            <td className="px-4 py-2.5">
                                                {!meSigner ? (
                                            ...
                                            <td className="px-2 py-2.5 relative" onClick={(e) => e.stopPropagation()}>
                                                <div className="flex items-center justify-end gap-1.5">
                                                    {needsMySignature && (
                                                        <button
                                                            ...
                                                            className="h-7 px-2 bg-slate-950 text-white text-[11px] font-medium rounded-md hover:bg-slate-800 transition-colors cursor-pointer flex items-center gap-1 shrink-0"
                                                        >
                                                            <BsPenFill size={10} />
                                                            Firmar
                                                        </button>
                                                    )}
                                                    {alreadySigned && doc.mySignature && (
                                                        <button
                                                            type="button"
                                                            onClick={() => setPreviewDoc(doc)}
                                                            aria-label="Ver firma"
                                                            className="w-7 h-7 rounded text-slate-400 hover:text-slate-700 hover:bg-slate-200 flex items-center justify-center cursor-pointer transition-colors"
                                                            title="Ver mi firma"
                                                        >
                                                            <BsEye size={12} />
                                                        </button>
                                                ...
                                                {isMenuOpen && (
                                                    <div ref={actionMenuRef} className="absolute right-2 top-full mt-1 w-52 bg-white border border-slate-200 rounded-md overflow-hidden z-10">
                                                        ...
                                                        />
                                                        {meCreator && (doc.status === 'sent' || doc.status === 'pending') && (
                                                            <>
                                                                <ActionMenuItem
            ...
            <AnimatePresence>
                {detailDoc && (
                    <DocumentDetailDrawer
                        ...
                        onSign={() => {
                            setDetailDoc(null);
                            setSigningDoc(detailDoc);
                        }}
                        onPreviewSig={() => {
                            setDetailDoc(null);
                            setPreviewDoc(detailDoc);
                        }}
                ...
                {signingDoc && (
                    <SignatureModal
                        doc={{
                            ...
                            sender: signingDoc.createdBy,
                            type: TYPE_LABEL[signingDoc.type],
                        }}
                        signerName={myName}
                ...
                {previewDoc && previewDoc.mySignature && (
...
function RoleBadge({ meCreator, meSigner }: { meCreator: boolean; meSigner: boolean }) {
    if (meCreator && meSigner) {
        return (
            <span className="inline-flex items-center gap-1 px-1.5 py-0.5 rounded text-[10px] font-medium bg-[#1e40af]/10 text-[#1e40af] border border-[#1e40af]/15">
                Creador y firmante
            </span>
        );
    }
...
function DocumentDetailDrawer({
    ...
    const status = STATUS_CONFIG[doc.status];
    const me = doc.signers.find((s) => s.email === myEmail);
    const meCreator = doc.createdBy === ME_NAME;
    const needsMySignature = me?.status === 'pending' && (doc.status === 'pending' || doc.status === 'sent');
    const alreadySigned = me?.status === 'signed';
                    ...
                    {needsMySignature && (
                        <div className="px-6 py-4 border-b border-slate-100 bg-amber-50/50">
                            <div className="flex items-start gap-2.5 mb-3">
                                ...
                                <div>
                                    <p className="text-[13px] font-semibold text-amber-900">Tu firma está pendiente</p>
                                    {doc.expiresAt && (
                                        <p className="text-[11px] text-amber-700 mt-0.5">Vence el {doc.expiresAt}</p>
                                    )}
                                </div>
                            </div>
                            <button
                                type="button"
                                onClick={onSign}
                                className="w-full h-9 bg-slate-950 text-white text-[13px] font-medium rounded-md hover:bg-slate-800 transition-colors cursor-pointer flex items-center justify-center gap-1.5"
                            >
                                <BsPenFill size={12} />
                                Firmar ahora
                            </button>
                    ...
                    {alreadySigned && doc.mySignature && (
                        <div className="px-6 py-4 border-b border-slate-100 bg-emerald-50/40">
                            <div className="flex items-start gap-2.5 mb-3">
                                ...
                                <div>
                                    <p className="text-[13px] font-semibold text-emerald-900">Ya firmaste este documento</p>
                                    {doc.mySignedAt && (
                                        <p className="text-[11px] text-emerald-700 mt-0.5">Firmado el {doc.mySignedAt}</p>
                                    )}
                                </div>
                            </div>
                            <button
                                type="button"
                                onClick={onPreviewSig}
                                className="w-full h-9 bg-white border border-slate-200 text-slate-700 text-[13px] font-medium rounded-md hover:bg-slate-50 hover:border-slate-300 transition-colors cursor-pointer flex items-center justify-center gap-1.5"
                            >
                                <BsEye size={12} />
                                Ver mi firma
                            </button>
                                ...
                                {doc.signers.map((s) => {
                                    ...
                                    return (
                                        <li key={s.email} className="flex items-start gap-2.5">
                                            <span className={`w-7 h-7 rounded-full border flex items-center justify-center text-[10px] font-semibold shrink-0 ${isMe ? 'bg-[#1e40af]/10 border-[#1e40af]/20 text-[#1e40af]' : 'bg-slate-100 border-slate-200 text-slate-600'
                                                }`}>
                                                {s.name.split(' ').map((w) => w[0]).slice(0, 2).join('').toUpperCase()}
                                            </span>
                                            <div className="min-w-0 flex-1">
                                                <p className="text-[13px] font-medium text-slate-900 truncate">
                                                    {s.name} {isMe && <span className="text-[10px] text-[#1e40af] font-semibold ml-1">(tú)</span>}
                                                </p>
                                                <p className="text-[11px] text-slate-500 font-mono truncate">{s.email}</p>
                                            </div>
                                            <div className="text-right shrink-0">
                                                <p className={`text-[11px] font-medium ${s.status === 'signed' ? 'text-emerald-700' : s.status === 'rejected' ? 'text-rose-700' : 'text-amber-700'}`}>
                                                    {s.status === 'signed' ? 'Firmado' : s.status === 'rejected' ? 'Rechazado' : 'Pendiente'}
                                                </p>
...
function SignaturePreviewModal({
    ...
    const handleDownload = () => {
        if (!doc.mySignature) return;
        const link = document.createElement('a');
        link.href = doc.mySignature;
                ...
                <div className="px-6 py-5">
                    ...
                    {doc.mySignedAt && (
                        <div className="flex items-center gap-1.5 mt-3 text-[11px] text-slate-500">
                            ...
                            Firmado el <span className="text-slate-700 font-medium">{doc.mySignedAt}</span>
                        </div>
                    )}
                </div>
