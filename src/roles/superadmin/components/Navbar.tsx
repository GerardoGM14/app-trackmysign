...
import type { ProfileData } from '../../../components/auth/ProfileModal';
...
const SECTIONS: SectionDef[] = [
    ...
    { label: 'Empresas', path: '/tenants', icon: <BsBuilding size={13} />, parent: 'Operación' },
    { label: 'Analítica', path: '/analytics', icon: <BsGraphUp size={13} />, parent: 'Negocio' },
    { label: 'Usuarios', path: '/users', icon: <BsPersonVcard size={13} />, parent: 'Negocio' },
    { label: 'Configuración', path: '/settings', icon: <BsShieldLock size={13} />, parent: 'Sistema' },
];
...
const INITIAL_NOTIFICATIONS: NotificationItem[] = [
    { id: '1', title: 'Nueva empresa registrada', body: 'EuroPrint Hub completó el onboarding.', time: 'hace 12 min', unread: true },
    { id: '2', title: 'Plan actualizado', body: 'Global Signage cambió a Enterprise.', time: 'hace 2 h', unread: true },
    { id: '3', title: 'Mantenimiento programado', body: 'Ventana sábado 23:00 UTC.', time: 'ayer', unread: false },
];
...
export default function Navbar({ sidebarOpen, setSidebarOpen }: NavbarProps) {
    ...
    const handleProfileSave = async (data: ProfileData) => {
        // Simulated persistence — replace with Firestore update when ready
    ...
    return (
        <>
            <header className="h-14 bg-white px-3 sm:px-4 flex items-center justify-between border-b border-slate-200 shrink-0 relative z-20 gap-3">
                ...
                {/* Breadcrumb con icono pegado a su izquierda */}
                ...
                <div className="hidden md:flex max-w-md flex-1 mx-2">
                    ...
                    >
                        ...
                        <span className="flex-1 text-left truncate">Buscar empresas, usuarios, planes...</span>
...
function ConnectivityBadge({ online }: { online: boolean }) {
    return (
        <div
            ...
            title={online ? 'Conectado a la plataforma' : 'Sin conexión a la plataforma'}
        >
...
function UserMenu({ user, avatarUrl, onProfile, onSettings, onShortcuts, onSignOut }: UserMenuProps) {
    return (
        <div role="menu" className="absolute right-0 mt-2 w-64 bg-white border border-slate-200 rounded-lg overflow-hidden z-30">
            ...
            <div className="py-1">
                <MenuItem icon={<BsPerson size={14} />} label="Ver perfil" onClick={onProfile} />
                <MenuItem icon={<BsGear size={14} />} label="Configuración de cuenta" onClick={onSettings} />
            ...
            >
                <div className="flex items-center gap-2.5 px-4 h-12 border-b border-slate-200">
                    ...
                    <input
                        ...
                        placeholder="Buscar páginas, empresas, usuarios..."
